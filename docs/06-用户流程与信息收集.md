...
| 步骤 | 字段 | 必填 | 存储建议 |
|------|------|------|----------|
| 1 项目类型 | `application_type`（不发光 logo / 发光 logo / lightbox / 侧面安装 logo） | 是 | `form_payload` |
...
| 6 物流安装 | `need_installation`, `installation_scene`（室内/室外）, `installation_method`（单个字母独立安装/整体 logo 或灯箱/金属横梁安装）, `country`, `postal_code`, `city` | 国家/邮编建议必填 | 同上 |
